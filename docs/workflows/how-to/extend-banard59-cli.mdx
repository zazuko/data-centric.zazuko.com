...
The pipeline `http://example.org/pipeline/tz` has two variables:

<CodeBlock language="turtle" title="pipeline/main.ttl">
  {selectLines(pipeline, { from: 6, to: 20 })}
</CodeBlock>

The will be mapped to optional CLI options:

```shell
barnard59 time-zone fetch --apiBase http://example.com/time --TZ Europe/Amsterdam
```
